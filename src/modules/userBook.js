...
export const searchStdBook = (keyword) => async (dispatch) => {
  try {
    const {
      data: { status, data },
    } = await axios.get(`${url}/stdbook/search_std_book?keyword=${keyword}`, {
...
export const uploadImg = (formData) => () => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      } = await axios.post(`${url}/upload/stdimg`, formData, {
        withCredentials: "true",
      });
      if (status === "OK") {
...
export const deleteStdBook = (stdb_code, navigate) => async (dispatch) => {
  try {
    const {
      ...
      `${url}/stdbook/delete_std_book?stdb_code=${stdb_code}`,
      { withCredentials: true }
    );
...
export const getMyBookList = (std_num) => async (dispatch) => {
  try {
    const {
      data: { status, message, data },
    } = await axios.get(`${url}/stdbook/get_my_book_list?std_num=${std_num}`, {
      withCredentials: true,
    });
...
export const getBorrowStdBookList = (std_num) => async (dispatch) => {
  try {
    const {
      ...
      `${url}/stdbook/mypage_std_borrow_list?std_num=${std_num}`,
      { withCredentials: true }
    );
...
export const getChatBook = (stdb_code) => async (dispatch) => {
  try {
    const {
      data: { status, data },
    } = await axios.get(`${url}/stdbook/get_chat_book?stdb_code=${stdb_code}`, {
      withCredentials: true,
    });
...
const userBook = (state = INIT_USERBOOK_STATE, { type, payload }) => {
  switch (type) {
    ...
    case DELETESTDBOOK:
      return {
        ...state,
        my_book_list: state.my_book_list.filter(
          ({ stdb_code }) => stdb_code !== payload
        ),
      };
    case GETMYBOOKLIST:
      return {
