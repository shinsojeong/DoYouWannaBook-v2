...
router.get("/admin_get_info", isLoggedIn, isAdmin, async (req, res) => {
  ...
  try {
    ...
    const location = await Libclass.findOne({
      where: { class_sign: book.libb_class },
    });
    return res.send({
      status: "OK",
      code: 200,
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      const exLibbCode = await Libbook.findOne({
        where: { libb_code },
      });
      if (exLibbCode) {
        return res.send({
          status: "ERR",
          code: 500,
          message: "청구 기호가 중복되었습니다.",
        });
      }

      const exClass = await Libclass.findOne({
